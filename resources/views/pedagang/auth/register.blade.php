    ...
    <div class="container">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{url('pedagang/daftar_handler')}}" method="post" enctype="multipart/form-data">
                    @csrf
        
                    <div class="form-floating mb-4">
                        <input type="text" class="form-control bg-white" id="floatingInput" name="nama_pedagang" value="{{$c->nama_customer}}" required readonly>
                        <label for="floatingInput">Nama Pedagang</label>
                    </div>
                    <div class="form-floating mb-4">
                    ...
                    <div class="form-floating mb-4">
                        <input type="number" class="form-control" id="floatingInput" required name="nomor_ktp" value="{{old('nomor_ktp')}}">
                        <label for="floatingInput" id="label_noktp">Nomor Ktp</label>
                        @if ($errors->has('nomor_ktp'))
                            <div class="text-danger text-small">
                                @foreach ($errors->get('nomor_ktp') as $err)
                                    {{ $err }}
                                @endforeach
                            </div>
                        @endif
